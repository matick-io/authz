...
import (
	...
	closure "github.com/matick-io/authz/materialize/postgres"
)
...
func openPool(t *testing.T) *pgxpool.Pool {
	...
	if err := closure.Migrate(ctx, pool); err != nil {
		t.Fatal(err)
	}
	return pool
}
...
func TestNestingBudget(t *testing.T) {
	pool := openPool(t)
	ds := postgres.New(pool, postgres.WithHook(mustNew(t, closure.WithNestingBudget(2)).Hook()))
	ctx := context.Background()
	...
	if !errors.Is(err, closure.ErrNestingTooLarge) {
...
func TestIndexRefusesForeignReaders(t *testing.T) {
	...
	if !errors.Is(err, closure.ErrNotPostgres) {
		t.Fatalf("got %v", err)
	}
}
...
func mustNew(t *testing.T, opts ...closure.Option) *closure.Index {
	t.Helper()
	idx, err := closure.New(opts...)
	if err != nil {
		t.Fatal(err)
	}
	return idx
